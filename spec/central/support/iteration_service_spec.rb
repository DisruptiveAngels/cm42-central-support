...
describe Central::Support::IterationService do
  let(:project) { create :project,
                  iteration_start_day: 2,
                  iteration_length: 1,
                  start_date: Time.zone.parse('2016-05-13') }
  let(:service) { Central::Support::IterationService.new(project) }
  ...
  context 'complete set of stories in many different iterations' do
    ...
    let(:service) { Central::Support::IterationService.new(project, current_time: today) }
    before do
      I18n.locale = :en
      Time.zone = 'Brasilia'
      ...
      allow_any_instance_of(Central::Support::IterationService).to receive(:fetch_stories!) { stories }
    end
    ...
    it '#group_by_day' do
      ...
      expect(groups.values).to eq([3, 5, 5, 10, 10, 18, 26, 26, 27, 27, 30, 38, 39, 47, 48, 51, 52, 60, 60, 63, 65, 66, 67, 72, 72, 77, 77, 77, 78, 78, 79, 84, 84, 89, 92, 93, 98, 106, 111, 116, 118, 126, 134, 134, 137, 145, 147, 152, 152, 160, 160, 162, 165, 165, 168, 171, 173, 181, 181, 181, 181, 181, 181, 181, 181, 181, 181, 181])
    end
